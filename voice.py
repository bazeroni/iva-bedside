## STT LANGUAGES ##

stt_language="en-US"

#stt_language="es-US"
#stt_language="es-MX"
#stt_language="es-PR"
#stt_language="es-DO"
#stt_language="es-SV"
#stt_language="es-CU"

#stt_language="yue-CN"
#stt_language="zh-CN"

#stt_language="vi-VN"

#stt_language="ru-RU"

#stt_language="ar-EG"
#stt_language="ar-SY"
#stt_language="ar-MA"

#stt_language="fr-FR"

#stt_language="km-KH"

#stt_language="it-IT"

#stt_language="fil-PH"

#stt_language="ja-JP"

## TTS LANGUAGES ##
# other than Aria, style compatible (-empathetic) with Davis, Guy, Jane, Jason, Jenny, Nancy, Tony

# ENGLISH #
#tts_voice='en-US-NancyNeural'
#tts_voice='en-US-JennyNeural'
tts_voice='en-US-AriaNeural'
#tts_voice='en-US-JennyMultilingualNeural'

# SPANISH #
#tts_voice='es-US-PalomaNeural' # united states
#tts_voice='es-MX-CarlotaNeural' # mexican
#tts_voice='es-PR-KarinaNeural' # puerto rican
#tts_voice='es-DO-RamonaNeural' # dominican
#tts_voice='es-SV-LorenaNeural' # salvadorean
#tts_voice='es-CU-BelkysNeural' # cuban

# CHINESE #
#tts_voice='yue-CN-XiaoMinNeural' # cantonese
#tts_voice='zh-CN-XiaochenNeural' # mandarin

# VIETNAMESE #
#tts_voice='vi-VN-HoaiMyNeural'

# RUSSIAN #
#tts_voice='ru-RU-DariyaNeural'

# ARABIC #
#tts_voice='ar-EG-SalmaNeural' # egyptian
#tts_voice='ar-SY-AmanyNeural' # syrian
#tts_voice='ar-MA-MounaNeural' # moroccan

# FRENCH #
#tts_voice='fr-FR-BrigitteNeural'

# KHMER #
#tts_voice='km-KH-SreymomNeural'

# ITALIAN #
#tts_voice='it-IT-ElsaNeural'

# TAGALOG #
#tts_voice='fil-PH-BlessicaNeural'

# JAPANESE #
#tts_voice='ja-JP-MayuNeural'